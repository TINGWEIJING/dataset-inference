...
def patch_attack_private(model1, model2, X, y, params, target1, target2):
    '''
    Broken function, function are not called in this codebase
    '''
    # ipdb.set_trace()
...
def loss_mingd(preds: torch.Tensor,
               target: torch.Tensor):
...
def rand_steps(model, X, y, args, target=None):
    ...
    def noise_2(X): return torch.normal(0, std, size=X.shape).cuda()
    def noise_1(X): return torch.from_numpy(np.random.laplace(loc=0.0, scale=scale, size=X.shape)).float().to(X.device)
    def noise_inf(X): return torch.empty_like(X).uniform_(-uni, uni)
...
def mingd(model: torch.nn.Module, X, y, args: Args, target) -> torch.Tensor:
    ...
    for t in range(args.num_iter):
        if t > 0:
            preds: torch.Tensor = model(X_r+delta_r)
